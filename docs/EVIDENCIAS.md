...
Bancos relacionais tradicionais foram desenhados em torno de uma máquina central coordenando tudo, com dados normalizados em tabelas conectadas por chaves estrangeiras. Esse modelo é excelente para consistência, mas escala mal horizontalmente: à medida que o volume de dados e a concorrência de acesso crescem, o custo de manter joins consistentes entre múltiplos servidores explode. Como o requisito central da Amazonas é justamente crescimento rápido com escalabilidade horizontal, a escolha recai sobre um banco de documentos (MongoDB), que guarda cada entidade como um documento autocontido (JSON/BSON), reduzindo a necessidade de joins na leitura; aceita esquema flexível por documento, então cada categoria de produto pode ter atributos diferentes sem alterar um schema global; e foi desenhado desde a origem para ser distribuído entre múltiplos servidores (sharding nativo).
...
O diagrama completo, com as seis entidades modeladas no Hackolade, está reproduzido por coleção na seção 2.3.

### 2.3 Detalhamento de cada coleção

#### 2.3.1 `clientes`

Guarda o cadastro de cada cliente: dados pessoais, contato e endereços. Os endereços são embutidos como um array — cada cliente tem no máximo alguns endereços cadastrados, e são sempre exibidos junto do cliente, nunca consultados isoladamente, o que torna o embedding a escolha natural. O histórico de pedidos não é embutido aqui: cresce sem limite ao longo dos anos e é consultado de formas diferentes (por status, por período), por isso vira a coleção própria `pedidos`, referenciando `cliente_id`.

Índices: `{email: 1}` único e `{cpf: 1}` único, para garantir não-duplicidade de cadastro e permitir busca rápida por esses campos.

![Coleção clientes no Hackolade](screenshots/modelo-clientes.png)
...
![Coleção produtos no Hackolade](screenshots/modelo-produtos.png)
...
![Coleção formas_pagamento no Hackolade](screenshots/modelo-formas-pagamento.png)

#### 2.3.4 `pedidos`

A coleção central da transação. `itens[]` fica embutido: sempre lido junto do pedido, tamanho limitado e nunca consultado isoladamente fora do contexto do pedido. `cliente_snapshot`, `endereco_entrega` e `forma_pagamento` são snapshots — cópias do dado no exato momento da compra. Isso é essencial, inclusive por motivos fiscais e de auditoria: se o cliente mudar de endereço ou trocar de cartão amanhã, o pedido antigo precisa continuar registrando fielmente como a compra realmente aconteceu. `cliente_id` é mantido como referência, permitindo consultas indexadas do tipo "todos os pedidos do cliente X" sem duplicar o cliente inteiro em cada pedido.

Índices: `{cliente_id: 1, numero_pedido: 1}` único (a unicidade em coleção sharded precisa ter a chave de shard como prefixo — ver seção 3), `{cliente_id: 1, data_pedido: -1}` (histórico do cliente, mais recente primeiro) e `{status: 1}` (filas operacionais, como "todos os pedidos aguardando envio").

![Coleção pedidos no Hackolade](screenshots/modelo-pedidos.png)

#### 2.3.5 `carrinho`

Separada de `pedidos` porque tem um perfil de acesso completamente diferente: é escrita a cada clique de "adicionar ao carrinho", ordens de magnitude mais frequente que a escrita de um pedido, e é efêmera — ou vira pedido, ou é abandonada. `itens[]` é embutido pelo mesmo raciocínio de `pedidos.itens[]`. Um índice TTL em `atualizado_em` remove automaticamente carrinhos abandonados após 30 dias, sem exigir um processo de limpeza externo.

Índices: `{cliente_id: 1}` único (um carrinho ativo por cliente) e `{atualizado_em: 1}` com `expireAfterSeconds` de 30 dias.

![Coleção carrinho no Hackolade](screenshots/modelo-carrinho.png)

#### 2.3.6 `avaliacoes`

Referencia `produto_id` (para listar todas as avaliações de um produto) e `cliente_id` (para auditoria/anti-fraude), mas guarda `cliente_nome` como snapshot, permitindo renderizar a lista de avaliações na tela de produto sem precisar buscar cada cliente individualmente.
...
![Coleção avaliacoes no Hackolade](screenshots/modelo-avaliacoes.png)
...
- **Replicação**: cópias idênticas do banco inteiro em vários servidores (um *replica set*, no MongoDB). Resolve disponibilidade — se o servidor primário cair, um secundário assume automaticamente. Não ajuda o banco a crescer em volume, já que cada réplica guarda o total dos dados.
...
Vale registrar uma limitação conhecida: hashear `produto_id` distribui bem entre produtos diferentes, mas não resolve um único produto "viral" que concentre volume desproporcional de avaliações. Isso é um *hot key* — um único valor de chave recebendo tráfego desproporcional — não um *hot chunk*, e o sharding por si só não resolve esse caso. Uma mitigação possível, fora do escopo deste modelo inicial, seria um cache de leitura na frente da coleção para produtos com pico de tráfego.

### 3.4 Lidando com crescimento de dados

O sharding nas coleções de maior volume (`pedidos`, `avaliacoes`) é o mecanismo primário de crescimento: em vez de um servidor cada vez maior, o cluster redistribui os dados automaticamente conforme novos shards são adicionados. O índice TTL em `carrinho` remove dado sem valor de negócio depois de 30 dias, controlando o crescimento de uma coleção com escrita muito frequente. Como evolução futura, fora do escopo mínimo deste desafio, pedidos muito antigos (por exemplo, com mais de 2 anos) poderiam ser movidos para um armazenamento de menor custo, mantendo a coleção operacional enxuta.

### 3.5 Lidando com alta concorrência de acesso

Escalar servidores não resolve, sozinho, um problema que aparece mesmo com um único documento: múltiplos clientes tentando alterar o mesmo dado ao mesmo tempo — típico em picos de tráfego, como Black Friday, quando centenas de pessoas tentam comprar a última unidade de um produto popular no mesmo segundo.

Se a aplicação executa "ler o estoque → decidir se há saldo → escrever o novo estoque" como passos separados, dois clientes podem ler o mesmo valor antes de qualquer um escrever, ambos concluírem que há saldo e ambos confirmarem a compra — vendendo a mesma unidade duas vezes. Esse erro é chamado de *race condition*. A correção não é ler mais rápido: é nunca separar a leitura da escrita quando a decisão depende do valor atual. A solução proposta é uma operação atômica condicional, algo equivalente a:
...
O próprio banco executa a leitura-verificação-escrita como um passo indivisível: apenas um dos clientes concorrentes consegue decrementar o estoque, e os demais recebem "esgotado" imediatamente, sem nenhuma janela de tempo em que os dois poderiam ter lido o mesmo valor.

### 3.6 Consistência sob falha

Quando a rede entre servidores falha — o que eventualmente acontece em qualquer sistema distribuído — um banco precisa escolher entre continuar respondendo com um dado possivelmente desatualizado (disponibilidade) ou recusar responder até ter certeza do dado mais atual (consistência); não é possível garantir as duas coisas simultaneamente durante a falha. Por padrão, o MongoDB prioriza consistência, já que leituras vão ao primary. Este plano propõe uma exceção deliberada: leituras que toleram alguns milissegundos de atraso, como listagem de avaliações e catálogo de produtos, podem ser direcionadas a secondaries, aliviando a carga do primary, enquanto leituras que exigem o dado mais atual, como confirmação de pagamento e verificação de estoque no checkout, permanecem no primary.
...
É o próprio MongoDB, hospedado e gerenciado pela MongoDB Inc., disponível nas três grandes nuvens (AWS, Azure, GCP). A migração deste modelo para o Atlas seria direta, porque o vocabulário — coleções, sharding, replica set — é idêntico ao usado neste documento. Réplica e sharding são configurados via interface gráfica ou infraestrutura como código, sem precisar operar manualmente os config servers e o roteador `mongos`; há auto-scaling de capacidade, aproximando o sistema da elasticidade que o enunciado cita como desejável; e backup automático, monitoramento e alertas já vêm prontos.

### 4.2 Amazon DynamoDB

Um banco NoSQL nativo da AWS, com um modelo um pouco diferente, misturando chave-valor e documento. O DynamoDB já é inerentemente particionado e replicado por baixo dos panos, sem shards ou replica sets para configurar manualmente; paga-se por unidades de leitura/escrita ou por uso no modo on-demand, em vez de dimensionar servidores; e a modelagem passa a ser por chave de partição e chave de ordenação — as coleções deste documento se traduziriam em tabelas, com `cliente_id` como chave de partição natural para `pedidos` e `carrinho`, e `produto_id` para `avaliacoes`. Índices secundários globais substituiriam os índices compostos do MongoDB, como o de `pedidos` por `status`.

### 4.3 Qual escolher

Para a Amazonas, MongoDB Atlas é a transição mais natural a partir deste documento, porque o modelo já foi desenhado no vocabulário do MongoDB. DynamoDB seria uma opção interessante se a prioridade fosse eliminar por completo a operação de infraestrutura de banco, aceitando reformular parte da modelagem em torno de chave de partição/ordenação e do modelo de consulta mais restrito do DynamoDB.

---

## Anexo A — Implementação opcional em MongoDB (bônus)

Como "ir além" opcional do enunciado, o modelo também foi implementado em Docker, com um replica set MongoDB de 3 nós populado com dados de exemplo. Detalhes de como rodar em [`implementacao-docker-mongodb.md`](implementacao-docker-mongodb.md).

O ambiente sobe um replica set real, mas não um cluster com sharding — isso exigiria config servers, múltiplos shards e um roteador `mongos`; a estratégia de particionamento já foi descrita e justificada na seção 3.

Evidências de que o banco está funcionando, a partir da raiz do repositório:

```bash
make demo              # reset completo + sobe a stack + espera o seed terminar + mostra status
make failover-test     # prova a alta disponibilidade
```
